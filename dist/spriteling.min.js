  ...
  var Spriteling = (function () {
      ...
      function Spriteling(options, element) {
          ...
          this.play = function (scriptName, animationObject) {
              if (animationObject === void 0) { animationObject = {}; }
              // Not yet loaded, wait...
              if (!_this._internal.loaded) {
                  setTimeout(function () { _this.play(scriptName, animationObject); }, 50);
                  return false;
              }
              if (scriptName) {
                  // Script provided
                  ...
                  if (typeof animationObject === 'object') {
                      var scriptName_1 = animationObject.script;
                      if (typeof scriptName_1 === 'string') {
                          animationObject.script = _this._internal.animations[scriptName_1];
                      }
                      if (typeof scriptName_1 === 'undefined') {
                          animationObject.script = _this._internal.animations['all'];
                      }
                      _this._log('info', "playing animation \"" + scriptName_1 + "\"");
          ...
          this._loadSpriteSheet = function () {
              ...
              _preload.src = _this._spriteSheet.url;
              imageLoaded(_preload, function () {
                  if (_this._internal.loaded) {
                      return;
                  } // <- Fix for some unexplained firefox bug that loads this twice.
                  _this._internal.loaded = true;
                  _this._log('info', 'loaded: ' + _this._spriteSheet.url + ', sprites ' + _this._spriteSheet.cols + ' x ' + _this._spriteSheet.rows);
                  _this._internal.sheetWidth = _preload.width;
                  _this._internal.sheetHeight = _preload.height;
                  _this._internal.frameWidth = parseInt(_this._internal.sheetWidth / _this._spriteSheet.cols, 10);
                  _this._internal.frameHeight = parseInt(_this._internal.sheetHeight / _this._spriteSheet.rows, 10);
                  _this._internal.totalSprites = (_this._spriteSheet.cols * _this._spriteSheet.rows) - _this._spriteSheet.cutOffFrames;
                  ...
                  _this._element.style.backgroundImage = "url(" + _this._spriteSheet.url + ")";
                  _this._element.style.backgroundPosition = '0 0';
                  if (_this._spriteSheet.top !== null) {
                      if (_this._spriteSheet.top === 'center') {
                          _this._element.style.top = '50%';
                          _this._element.style.marginTop = _this._internal.frameHeight / 2 * -1 + "px";
                      }
                      else {
                          _this._element.style.top = _this._spriteSheet.top + "px";
                      }
                  }
                  if (_this._spriteSheet.right !== null) {
                      _this._element.style.right = _this._spriteSheet.right + "px";
                  }
                  if (_this._spriteSheet.bottom !== null) {
                      _this._element.style.bottom = _this._spriteSheet.bottom + "px";
                  }
                  if (_this._spriteSheet.left !== null) {
                      if (_this._spriteSheet.left === 'center') {
                          _this._element.style.left = _this._spriteSheet.left + "px";
                  ...
                  // Starting sprite?
                  if (_this._spriteSheet.startSprite > 1 && _this._spriteSheet.startSprite <= _this._internal.totalSprites) {
                      _this.showSprite(_this._spriteSheet.startSprite);
                  }
                  // onLoaded callback
                  if (typeof _this._spriteSheet.onLoaded === 'function') {
                      _this._spriteSheet.onLoaded();
                  }
              });
          };
          /**
          ...
          this._drawFrame = function (frame) {
              ...
              var row = Math.ceil(frame.sprite / _this._spriteSheet.cols);
              var col = frame.sprite - ((row - 1) * _this._spriteSheet.cols);
              var bgX = ((col - 1) * _this._internal.frameWidth) * -1;
              var bgY = ((row - 1) * _this._internal.frameHeight) * -1;
              if (row > _this._spriteSheet.rows || col > _this._spriteSheet.cols) {
          ...
          this._log = function (level, message) {
              if (level === 'info' && !_this._spriteSheet.debug)
                  return;
